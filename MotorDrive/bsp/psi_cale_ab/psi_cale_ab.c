...
{
        vary.a_acc_d=20.0f;  //加加速度值 acc=-100t+20  
		vary.a_acc_d_l=0;
        vary.a_m=0;
        vary.acc_time=1.0f;//加速时间，被除数，不能为0
        vary.dcc_time=1.0f;//减速时间，
    
        vary.acc_d=20.0f;   //加速度值
		vary.acc_d_l=20.0f;
        vary.omega_d=0;   //速度值
		vary.omega_d_l=0; 
		vary.theta_d=0;   //位置值
        ...
        vary.new_move=false;
        vary.acc_flag=false;
        vary.theta_pin=0; //储存起步时位置
        vary.theta_acc=0; //加速阶段位移圈速
        vary.speed=0;//速度模式时目标速度
        vary.distance=1.34f;//位置模式时目标位置 6r/s:7.6;2r/s:1.34

    
		vary.omega_s=0;   //omega_*
		vary.omega_s_l=0;   //omega_*_last 用于求*的导数
	
		vary.tau_d=0.3f;   //tau_d  1=4000/2pi 由于根据算法调节所得，可令1=512
		vary.theta=0;   //theta
        vary.omega=0;   //omega
        vary.omega_e=0;
        vary.omega_e_l=0;
		...
		vary.ia_d=0;   
        vary.ia_d_l=0;
		vary.ia_oi=0;	//ia的积分输出项
		vary.ia_oi_l=0;	//ia的积分输出项last
		vary.ia_e=0;	//ia的误差值
		vary.ia_e_l=0;	//ia的上一步误差值
        vary.ia_od=0;
		vary.ud=0;	
        vary.ua_out=0;	
        vary.um=0;
